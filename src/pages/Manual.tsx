      ...
      <div className="manual-root">
        ...
        <section className="mb-10 page-break">
          ...
          <div className="mb-6">
            ...
            <ol className="space-y-3">
              {[
                { step: 1, title: 'カレンダーを開く', desc: '上部ナビの「📅 カレンダー」をタップします。Googleカレンダーのような時間グリッドが表示されます。' },
                { step: 2, title: 'ドラッグで空き枠を追加する', desc: 'グリッド上を上から下にドラッグ（またはタップ）するだけで空き枠が即時追加されます。ドラッグ中に時間帯がリアルタイムで表示されます。営業時間外のエリアも自由に設定できます。' },
                { step: 3, title: '週次一括追加を使う', desc: '「週次設定」ボタンから、同じ時間帯を指定曜日にまとめて数週分登録できます。定期的な稼働がある場合に便利です。' },
                { step: 4, title: '枠をタップして詳細・削除', desc: '追加済みのブロックをタップすると詳細（予約内容・患者情報・住所）が表示されます。予約のない枠は「この枠を削除する」で削除できます。' },
              ].map(({ step, title, desc }) => (
                <li key={step} className="flex gap-3">
                  <span className="w-6 h-6 rounded-full bg-teal-100 text-teal-700 text-xs flex items-center justify-center font-bold flex-shrink-0 mt-0.5">{step}</span>
                  <div>
          ...
          <div className="space-y-4">
            {[
              ...
              },
              {
                ...
                a: 'カレンダーグリッドをドラッグして枠を追加した後、MSWの検索条件（日時・使用機材・エリア）と一致しているかご確認ください。プロフィールで対応エリアと対応機材が正しく設定されているかもあわせてご確認ください。',
              },
              {
