...
const SCENES: Scene[] = [
  ...
  },
  {
    ...
    role: '事業所',
    roleColor: 'bg-teal-600',
    goal: '「空き枠の追加・削除がカレンダー上で直感的にできる」を見せる',
    steps: [
      {
        action: '「カレンダー」を開く',
        where: 'ナビ「📅 カレンダー」',
        point: '週表示で登録済みの空き枠が一覧できる',
      },
      {
        action: '空き枠を追加する',
        where: '上部フォームで日付・時間を選択 → 「＋ 追加」',
        point: '追加した瞬間から MSW の検索結果に表示される',
      },
      {
        action: '週次一括追加を試す（本番機能）',
        where: '「週次一括追加」セクションで曜日・時間を設定',
        point: '定期的な空き枠を一括登録できる。毎週月・水・金など設定可能',
      },
      {
        action: '不要な枠を削除する',
        where: '枠カードの「削除」ボタン → 「確認」ボタン',
        point: '2段階確認で誤削除を防止。確定済み予約のある枠は削除不可',
      },
    ],
  },
]
