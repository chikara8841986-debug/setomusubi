...
-- 放置pendingナッジの重複防止フラグ
ALTER TABLE public.reservations
  ADD COLUMN IF NOT EXISTS pending_reminder_sent boolean NOT NULL DEFAULT false;

-- approve_reservation に「同一車両・同時間帯の確定済みが既にある場合は弾く」再チェックを追加
-- （同時申請レースで pending が2件残ったとき、両方承認されるのを防ぐ）
