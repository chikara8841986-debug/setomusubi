...
- **該当**: `src/pages/msw/Search.tsx`（フォームの使用機材セレクタ 806行付近、handleSubmitRequest）
- **修正方針**: 選択肢は2つ。(a) フォームの機材を検索時の機材に固定（変更したければ再検索させる。シンプルで安全、推奨）。(b) 機材変更時に availableVehicles を再判定し、該当車両ゼロなら申請ボタンを無効化して「この機材に対応する空き車両がありません。条件を変えて再検索してください」を表示。加えて防御として、`vehicleId` が null になる場合は申請を中断するガードを入れる。
- **検証**: 検索=車椅子→フォームでストレッチャーに変更→申請できない（or 再検索誘導）こと。
