...
- [ ] **G1（GENERAL_USERS_PLAN §4/§5 フロント）— 2026-08-24 大部分をブラウザで検証済み、残りは下記参照**: 個人利用者向けフロントを実装し、開発サーバー上で実際に `/register/personal` から `role='personal'` で新規登録 → `/login` からログイン → `/my/search` で事業所検索 → 予約申請 → `/my/reservations` で一覧表示 → `cancel_reservation_by_personal` RPC でキャンセル、までブラウザ操作で確認済み（検証方法は本ファイル末尾「G. 一般利用者への開放」参照）。**未確認**: (1) 事業所プロフィール画面の「個人予約を受け付ける」チェックボックスを実際にクリックして保存できること（事業所アカウントの認証情報が無く未実施。DB直接更新での同等効果は確認済み）、(2) 本番のメール確認リンクを実際にクリックしての登録完了、(3) 事業所側が個人予約を承認したときの確定メールが申込者に届くこと。
...
- **調査結果 2026-07-18**: 日本のインボイス制度(2023-10-01〜)に対応するには、Stripeダッシュボードの「請求書の設定」(`/settings/billing/invoice`)で自社の適格請求書発行事業者番号(Tナンバー)を「JP TRN」として登録すれば、Stripe Billing経由で発行される請求書PDF・領収書に自動的に反映される([Stripe公式ガイド](https://stripe.com/guides/best-practices-for-setting-up-invoicies-in-japan))。設定はダッシュボード上の操作のみで、コード変更は不要と見られる。
  - **前提条件**: そもそも自社が適格請求書発行事業者として税務署に登録済み（Tナンバー取得済み）であることが必要。未登録なら先にそちらの手続きが要る（Claudeが代行できない税務手続き）。
  - **未確認**: 実際のStripeダッシュボードの設定画面の場所は、Stripe公式ドキュメント側の記載がやや古い可能性があるため、ダッシュボードにログインして`設定→請求書`まわりで実際の項目名を確認する必要がある。Tナンバー登録済みなら人間の作業として数分で終わる想定。
  - **次のアクション**: (1) 自社がTナンバー登録済みか確認、(2) 登録済みならStripeダッシュボードで「JP TRN」相当の項目にそのナンバーを入力、(3) テスト決済で発行される請求書に番号が載ることを確認。いずれも人間の作業（Stripeダッシュボード操作・税務確認）で、コード対応は不要と判断。
...
- **項目の洗い出し 2026-07-18（ユーザー判断: 今は項目整理のみ、実作業はローンチが近づいてから）**:
  1. **Stripe live鍵への切替**: `STRIPE_SECRET_KEY`・`STRIPE_WEBHOOK_SECRET`をtestモードからliveモードのものに再作成し、Supabase Edge Function Secretsを更新。切替後は`stripe-webhook`のWebhook URLをlive用に再登録し直す必要あり（C1のCodex最終GOと合わせて実施が望ましい）。
  2. **C2（正規Price ID設定）**: 今回「ローンチ直前まで後回し」と決定済み。C7実施時にセットで対応。
  3. **Billing Portalの日本語化・解約導線の設定確認**: Stripeダッシュボードの「顧客ポータル」設定で、表示言語・解約可否・キャンセルタイミングが、C4で規約に明記した内容（期間末解約・日割り返金なし）と一致しているか確認。
  4. **振込口座・本人確認（Stripe側のアカウント確認）**: Stripeダッシュボードの「アカウント」設定で、本番入金先口座の登録・本人確認(KYC)ステータスが完了しているか確認。未完了だと入金が止まる。
  5. **C5（インボイス対応）**: Tナンバー登録済みなら、このタイミングでStripeダッシュボードに登録。
  6. **テストデータの掃除**: 本番DBに残っているテスト事業所・テスト予約・テストMSWアカウント等を削除（今回文字化け確認で使われた「テスト病院」もここに含まれる）。
  7. **E1（Supabase Proプラン化）**: 今回「まだ無料のままでいい」と決定済み。長期間アクセスが空くと停止するリスクがあるため、ローンチ直前にPro化を再検討。
  - **未実施**: 上記はあくまで項目の洗い出し。実際の切替作業（live鍵の再作成等）はユーザーの指示があってから着手する。
...
### [ ] E4. バス係数1 — 現状確認済み・対応は見送り 2026-07-04
...
### [x] F3. LINE通知 — 自分のLINE連携まで対応済み 2026-07-14
- **対応内容**:
  - LINE公式アカウント「せとむすび」(@537bqcvu)をユーザー作業で作成。プロバイダー「せとむすび」で新規作成し、Messaging APIを有効化（利用規約同意はユーザー確認のうえ実施）。プライバシーポリシー/利用規約URLも登録済み。
  - あいさつメッセージは当初「せとむすび」「ハコビテ」「ふじ賃貸安否確認」の3アカウントすべてでオフにしたが、その後ユーザーからのフィードバックを受け「せとむすび」のみ再度オンにし、内容を「連携コードをこのトーク画面に送信してください」という案内文に差し替えた（連携操作の分かりやすさのため）。ハコビテ・ふじ賃貸安否確認はオフのまま。
  - 「せとむすび」で連携コード送信時にLINE標準の定型自動応答（「ただいまお問い合わせを受け付けておりません」等）が二重に返っていた原因が`応答メッセージ`機能（Webhookとは別の、LINE側のキーワード自動応答）だったと特定し、オフに変更。ハコビテ・ふじ賃貸安否確認の同設定は未確認・未変更（必要なら別途対応）。
  - `line-webhook`の返信文面を「せとむすびとのLINE連携が完了しました！🎉 以降、予約の申請・承認・キャンセルなどの通知がLINEにも届くようになります。」のように、何が届くか分かる温かみのある文面に変更（v3としてデプロイ）。
  - `LINE_CHANNEL_ACCESS_TOKEN`・`LINE_CHANNEL_SECRET`をSupabase Edge Function Secretsに登録済み（値はユーザー本人が入力。Claudeは値を扱っていない）。
  - `profiles.line_link_code`/`line_link_code_expires_at`を追加し、`generate_line_link_code()` RPC（SECURITY DEFINER、10分有効の6桁コード発行）を新設。
  ...
  - **追補 2026-07-18（ユーザーが実際にMSWアカウントで仮予約を申請し、事業所へのLINE通知本文で発覚）**:
    - **件名の二重表示**: `notify`(通知配送層)の`sendLinePush`が、本文(`text`)に既に埋め込まれている「【せとむすび】〇〇」という見出し行に加えて、`subject`をさらに`【】`で囲んで前置していたため、LINEメッセージの先頭に見出しが二重に表示されていた（例:「【【せとむすび】新しい仮予約申請が届きました】」の直後に「【せとむすび】新しい仮予約申請が届きました」）。原因はLINE/メール共通の`dispatch`関数に、本文が既に自己完結した見出しを持つ設計であることを踏まえずsubjectを再度前置する処理が残っていたこと。`sendLinePush`をtextをそのまま送るよう修正（`notify` v7としてデプロイ・本番照合済み）。件名二重表示は全ての通知種別（予約確定・却下・キャンセル・リマインド等）に共通する構造上の問題だったため、これで一括解消。
    - **「仇予約」の文字化け**: `send-request-received`(新規予約申請の通知)の本番デプロイ済みコードだけ、A5対応時と同じ原因（過去デプロイ時の書き写しミス）で「仮予約」が「仇予約」に化けたまま残っていた。repoのファイル自体は正しかったため、そのままv18として再デプロイし、本番の中身を取得して「仮予約」に戻ったことを照合済み。
    - **自己点検で追加発見**: 上記`notify`修正のデプロイ作業中、Unicodeエスケープへの手動変換でさらに1箇所（「Bearerに付けて叩く」の「叩く」→「叛く」）を新規に文字化けさせてしまい、直後の本番照合で発見・v7再デプロイで是正した。手打ちでの文字起こしは今後も再発しうるため、Edge Functionのデプロイでは**デプロイ後に必ず`get_edge_function`で本番の中身を取得し、日本語文言をrepoと目視突合してから完了扱いにする**運用を徹底する（ハブのエラー辞書にも追記済み）。
...
## 🟤 G. 一般利用者への開放（GENERAL_USERS_PLAN.md フェーズ3）

### [x] G1. 個人利用者向けフロントエンド実装 — 実装・ブラウザ検証済み 2026-08-24
- **実装**: `/register/personal`（新規登録）、`/my/search`（事業所検索・予約申請）、`/my/reservations`（予約一覧・キャンセル）、`/my/profile`（マイページ）を新設。`/login` を利用者向けに、`/business/login` を事業者向けに分割。`business/Profile.tsx` に `accepts_personal_requests` トグルを追加。サイト全体のMSW限定表現を修正（詳細は `GENERAL_USERS_PLAN.md` 末尾のファイル一覧を参照）。
- **検証方法**: 開発サーバー（`npm.cmd run dev`、ローカルDBではなく本番Supabaseプロジェクト `lcuoeekhnmbhomcdbedi` に接続）を実際にブラウザで操作。
  1. `/register/personal` から実際にアカウント登録 → `auth.users.raw_user_meta_data->>'role'='personal'`、`profiles.role='personal'`、`hospitals`/`businesses` 行が作られないことをSQLで確認。
  2. メール確認は開発上省略するため `email_confirmed_at` をSQLで直接セット（実際の確認メールのリンク押下は未検証 → 上記G1の「未確認」参照）。
  3. `/login` からログイン → `role='personal'` で `/my/search` へ自動リダイレクトされること、ヘッダーに登録時の氏名（`user_metadata.full_name`）と「ご利用者」バッジが表示されることを確認。
  4. `/my/search` で検索 → 個人予約を受け付けている事業所が無い状態では「条件に合う事業所は見つかりませんでした」の空状態が正しく出ることを確認。
  5. 検証用に既存事業所（ハコビテ、`id=fd98f752-...`）の `accepts_personal_requests` を一時的に `true`、`subscription_status` を `trialing` にSQLで変更 → 再検索すると検索結果に表示されることを確認（画面のフィルタとRLSの両方が正しく連動していることの確認）。予約申請フォームに病棟・病室欄が無いこと、電話番号欄に登録時の電話番号が自動入力されることを確認。
  6. 実際に予約申請を送信 → `reservations.source='personal'`、`hospital_id=null`、`requester_user_id`=ログイン中のuser.idであることをSQLで確認。`/my/reservations` に一覧表示されることを確認。
  7. 予約詳細を開き「キャンセル」→確認ダイアログ→実行 → `cancel_reservation_by_personal` RPC が呼ばれ `status='cancelled'` になること、`occupied_slots` が解放されていることをSQLで確認。画面上も「過去の予約」タブへ移動することを確認。
  8. 検証終了後、事業所の `accepts_personal_requests`/`subscription_status` を元の値（`false`/`canceled`）に戻し、テスト用の予約・ユーザー（`auth.users`→`profiles`カスケード削除）を削除して本番データを検証前の状態に復元済み。
- **ビルド**: `npm.cmd run build` 成功（`tsc -b && vite build`、warningのみでエラーなし）。
- **未確認（人間による最終チェック待ちに反映済み）**: 事業所プロフィール画面のチェックボックスUIそのもののクリック確認（事業所アカウントのログイン情報が無いため未実施）、実際の確認メールのリンク経由での登録完了、個人予約が承認されたときの確定メール到達。
